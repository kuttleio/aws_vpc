...
module "aws_internet_gateway" {
  ...
  tags           = merge(var.tags, map("Name", "${var.vpc_name}-igw"))
...
resource "aws_subnet" "public_subnets" {
  ...
  tags              = merge(var.tags, map("Name", "${var.vpc_name}-${var.enable_internet_gateway == "true" ? "public-${count.index + 1}" : "private-${count.index + 1 + (var.az_count * (var.enable_private_subnets == "true" ? 1 : 0))}"}"))
}
...
resource "aws_route_table" "public_route" {
  count  = var.az_count * (var.enable_public_subnets == "true" ? 1 : 0)
  vpc_id = aws_vpc.vpc.id
  tags   = merge(var.tags, map("Name", "${var.vpc_name}-${var.enable_internet_gateway == "true" ? "public-rt-${count.index + 1}" : "private-rt-${count.index + 1 + (var.az_count * (var.enable_private_subnets == "true" ? 1 : 0))}"}"))
}
...
resource "aws_route_table" "private_route" {
  count  = var.az_count * (var.enable_private_subnets == "true" ? 1 : 0)
  vpc_id = aws_vpc.vpc.id
  tags   = merge(var.tags, map("Name", "${var.vpc_name}-private-rt-${count.index + 1}"))
}
...
module "aws_eip" {
  ...
  tags           = merge(var.tags, map("Name", "${var.vpc_name}-nat-eip"))
}
...
resource "aws_nat_gateway" "ngw" {
  ...
  tags          = merge(var.tags, map("Name", "${var.vpc_name}-nat-${count.index + 1}"))
}
...
resource "aws_vpc_endpoint" "s3_endpoint" {
  ...
  tags         = merge(var.tags, map("Name", "${var.vpc_name}-endpoint-to-s3"))
}
...
resource "aws_vpn_gateway" "vpn_gw" {
  ...
  tags            = merge(var.tags, map("Name", "${var.vpc_name}-vgw"))
}
